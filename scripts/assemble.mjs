...
/**
 * Common OCR truncation/misread corrections for Japanese text.
 * Same dictionary as synthesize.mjs — applied to final markdown output
 * to catch any remaining OCR artifacts in non-table sections.
 * @type {Array<{pattern: RegExp, replacement: string}>}
 */
const OCR_CORRECTIONS = [
  { pattern: /エクス一卜/g, replacement: 'エクスポート' },
  { pattern: /エクスポ一卜/g, replacement: 'エクスポート' },
  { pattern: /エクス一ト/g, replacement: 'エクスポート' },
  { pattern: /イン一卜/g, replacement: 'インポート' },
  { pattern: /インポ一卜/g, replacement: 'インポート' },
  { pattern: /イン一ト/g, replacement: 'インポート' },
  { pattern: /テナ卜/g, replacement: 'テナント' },
  { pattern: /テナン卜/g, replacement: 'テナント' },
  { pattern: /ユザー/g, replacement: 'ユーザー' },
  { pattern: /セグメン卜/g, replacement: 'セグメント' },
  { pattern: /セグメソト/g, replacement: 'セグメント' },
  { pattern: /アカウン卜/g, replacement: 'アカウント' },
  { pattern: /デフォル卜/g, replacement: 'デフォルト' },
  { pattern: /チェッ夕/g, replacement: 'チェック' },
  { pattern: /リクエス卜/g, replacement: 'リクエスト' },
  { pattern: /コメン卜/g, replacement: 'コメント' },
  { pattern: /マスタ一/g, replacement: 'マスター' },
  { pattern: /デー夕/g, replacement: 'データ' },
  { pattern: /ソー卜/g, replacement: 'ソート' },
  { pattern: /ポイン卜/g, replacement: 'ポイント' },
  { pattern: /segmentld/g, replacement: 'segmentId' },
  { pattern: /tenantld/g, replacement: 'tenantId' },
  { pattern: /brigadeld/g, replacement: 'brigadeId' },
  { pattern: /hospitalld/g, replacement: 'hospitalId' },
  { pattern: /staffld/g, replacement: 'staffId' },
...
/**
 * Apply OCR text corrections to the complete markdown output.
 * Deterministic find-replace for common PaddleOCR misreads.
...
function applyOcrCorrections(text) {
  ...
  if (totalFixes > 0) {
    console.log(`  🔤 OCR text corrections: ${totalFixes} fixes applied to output`);
  }
  return corrected;
}

/**
 * Priority keywords for auto-assignment when LLM returns 'unknown'.
 * Maps Japanese/Vietnamese keywords to priority levels.
 * @type {Array<{pattern: RegExp, priority: string}>}
 */
...
/**
 * Auto-assign priority to a requirement based on keyword analysis.
 * Only assigns when priority is 'unknown' or missing.
 * @param {string} description - Requirement description text
 * @returns {string} Detected priority or 'unknown'
 */
function detectPriority(description) {
  if (!description) return 'unknown';
  for (const { pattern, priority } of PRIORITY_KEYWORDS) {
    if (pattern.test(description)) return priority;
  }
  return 'unknown';
}
...
function formatRequirements(requirements) {
  ...
  return sorted
    .map((req) => {
      // Auto-assign priority if unknown
      let priority = req.priority || 'unknown';
      if (priority === 'unknown') {
        priority = detectPriority(req.description);
      }
      const priorityTag = priority && priority !== 'unknown' ? ` [${priority.toUpperCase()}]` : '';
      return `- **${req.id || 'REQ-???'}**${priorityTag}: ${req.description || 'N/A'}`;
    })
    .join('\n');
}
 ...
 */
function generateMarkdown(synthesis, manifest) {
...
## Phụ lục: Nguồn Evidence

> Chi tiết Evidence được xuất ra file riêng: **evidence-appendix.md**
> File chứa toàn bộ OCR blocks, confidence scores, và page images.
...
async function main() {
  ...
  // Apply OCR text corrections to the full markdown output
  const markdown = applyOcrCorrections(generateMarkdown(synthesis, manifest));
  ...
  // Write evidence appendix as separate file
  const appendixContent = generateAppendix(outputDir);
  if (appendixContent && !appendixContent.includes('Không có dữ liệu OCR')) {
    const appendixPath = path.join(outputDir, 'evidence-appendix.md');
    const appendixMd = `# Evidence Appendix\n\n> **Document ID:** ${manifest.docId || 'Unknown'}\n> **Source:** ${manifest.sourceName || 'Unknown'}\n\n---\n\n${appendixContent}\n`;
    fs.writeFileSync(appendixPath, appendixMd, 'utf-8');
    console.log(`  ✅ Evidence appendix: ${appendixPath}`);
  }
