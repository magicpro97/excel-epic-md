...
import { buildOoxmlByName, formatOoxmlForPrompt, sheetHasOoxmlData } from '../loaders/ooxml-loader.mjs';
...
async function processPage(client, pageNumber, ocrData, renderPagesDir = null, visionClient = null, ooxmlSheet = null) {
  ...
  if (!ocrData.blocks || ocrData.blocks.length === 0) {
    ...
    if (hasOoxmlShapes) {
      log(
        'info',
        `📦 Page ${pageNumber}: 0 OCR blocks but ${ooxmlSheet.shapes.length} OOXML shapes → using OOXML data`,
      );
...
async function processIndividualPages(client, pagesToProcess, ocrDir, summariesDir, ooxmlData = new Map()) {
  ...
  // Build pageNumber → sheetName map from manifest, then resolve via name-based OOXML lookup
  // (fixes mismatch where manifest uses sequential indices but OOXML uses actual Excel tab indices)
  const pageToSheetName = new Map();
        ...
        for (const sheet of manifest.sheets) {
          if (sheet.sheetName && Array.isArray(sheet.sourcePages)) {
            for (const pageNum of sheet.sourcePages) {
              pageToSheetName.set(pageNum, sheet.sheetName);
            }
          }
        }
      }
    } catch {
      /* manifest read failed — no OOXML mapping */
    }
  }
  ...
  for (let i = 0; i < pagesToProcess.length; i += concurrency) {
    ...
    const batchPromises = batch.map(async (page) => {
      ...
      try {
        const sheetName = pageToSheetName.get(pageNumber);
        const ooxmlSheet = sheetName ? ooxmlByName.get(sheetName) || null : null;
        const hadOoxml = sheetHasOoxmlData(ooxmlSheet);
...
export { collectTablesFromPageSummaries, processIndividualPages, processPage };
